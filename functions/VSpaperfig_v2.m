function VSpaperfig_v2(stats_oneperunit_stacked,stats_stacked, stats_oneperunit_BMF_stacked, stats_selected_BMF_stacked, coreoptions)
% VS paper fig v2.
...
plotopts = {'figh',figh, ...
            'rowtitle','a. MTF-VS at low sound levels', ...
...
legend off;
xlabel(''); 
ylabel('VS / VS_m_a_x','fontweight','bold','fontsize',8);
...
set(gca,'xtick',[]);
ylim([20 1e3]); box off;  text(.75,1.1e3,'b. BMF-VS','fontweight','bold');
...
mtftypes = unique(stats_selected_BMF_stacked.VS_MTFType);
...
for ui = 1:length(unittypes)
    for li = 1:length(levels)
       vsmtftype_unittype = stats_selected_BMF_stacked.VS_MTFType( ...
            strcmp(stats_selected_BMF_stacked.rationalisedType,unittypes{ui}) & ...
            stats_selected_BMF_stacked.modLevel>=(levels(li)-10) & stats_selected_BMF_stacked.modLevel<(levels(li)+10));
      ...
      dpmtftype_unittype = stats_selected_BMF_stacked.dprime_MTFType( ...
          strcmp(stats_selected_BMF_stacked.rationalisedType,unittypes{ui}) & ...
          stats_selected_BMF_stacked.modLevel>=(levels(li)-10) & stats_selected_BMF_stacked.modLevel<(levels(li)+10));
...
text(0, maxv*1.1*1.1,'c. MTF-VS classification','fontweight','bold');
xlabel('Neuron type/Sound level','fontweight','bold');
text(4.5,12,'30','fontsize',7); text(5.5,18,'50','fontsize',7); text(6.5,16,'70','fontsize',7);
text(5,22,'dB SPL','fontsize',7);
text(2.9,8,'bandpass','rotation',90,'fontsize',7);
text(12.9,1,'lowpass','color','w','rotation',90,'fontsize',7);
ylim([0 maxv*1.1])
...
bh = boxplot( stats_selected_BMF_stacked.VS, ...`
...
for bi = 1:size(bh,2)
    col = get(bh(2,bi),'Color');
    col(col==0) =  0.3*rem(bi-1,3);
    set(bh(2,bi),'Color',col);
end
...
box off;
xlabel('Neuron type/sound level','fontweight','bold');
ylabel('VS at BMF','fontweight','bold'); ylim([0.1 1]);
text(1,1.1,'f. VS maximum','fontweight','bold');
